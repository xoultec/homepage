    ...
    >
      <div className="modal-box w-11/12 max-w-4xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex justify-between items-center mb-6 border-b border-base-300 pb-4 flex-shrink-0">
          <h3 id={`product-${product.id}-title`} className="font-bold text-2xl pr-4">
            {product.name}
          </h3>
          <button
            ref={closeButtonRef}
        ...
        <div className="flex-1 overflow-y-auto lg:overflow-hidden">
          ...
          <div className="lg:overflow-y-auto lg:h-full" id={`product-${product.id}-description`}>
            <div className="space-y-6">
              {/* Description */}
              <div>
                <h4 className="text-lg font-semibold mb-3">Description</h4>
                <p className="text-base-content/80 leading-relaxed">
                  {product.description || 'No description available for this template.'}
                </p>
              </div>
              ...
              {product.variants && product.variants.length > 0 ? (
                <div>
                  <h4 className="text-lg font-semibold mb-3">License Options</h4>
                  <div className="space-y-3">
                    {product.variants.map((variant) => (
                      <div key={variant.id} className="bg-base-200 p-4 rounded-lg">
              ...
              <div className="bg-info/10 border border-info/20 rounded-lg p-4">
                <h5 className="font-semibold text-base-content mb-2">✨ Premium Support</h5>
                <p className="text-sm text-base-content/70">
                  Get help with installation, customization, and any questions you might have. 
                  Email support is included with your purchase.
                </p>
              </div>
            </div>
          </div>
